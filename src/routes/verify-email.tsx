...
function VerifyEmailPage() {
  ...
  useEffect(() => {
    if (!loading && user?.email_confirmed_at) navigate({ to: "/" });
  ...
  const recheck = async () => {
    ...
    if (data.user?.email_confirmed_at) {
      ...
      toast.error("Not verified yet. Click the link in your email, then try again.");
    }
  };
    ...
    <main className="flex min-h-screen items-center justify-center bg-gradient-to-b from-background via-background to-primary/5 px-4 py-10">
      <Card className="w-full max-w-md overflow-hidden p-0">
        <div className="relative flex flex-col items-center bg-gradient-ocean px-6 py-8 text-primary-foreground">
          ...
          <img src={insightlyIcon} alt="Insightly" className="h-14 w-14 object-contain" />
          <h1 className="mt-3 font-display text-xl font-bold">Verify your email</h1>
          <p className="mt-1 text-center text-xs opacity-90">
            We sent a confirmation link to
            <br />
            <span className="font-semibold">{user?.email ?? "your inbox"}</span>
          </p>
        </div>
